...
def extract_packet_window_dataset_instances(traffic_capture: pd.DataFrame) -> pd.DataFrame:
    ...
    for start in range(0, len(traffic_capture), WINDOW_SIZE):
        ...
        # each window frame should contain the exact value described in the window
        ...
        dataset_row = {
            ...
            'avg_inter_packet_time': inter_packet_times.mean() if len(inter_packet_times) > 0 else 0,
            'inter_packet_time_dev': inter_packet_times.std() if len(inter_packet_times) > 0 else 0,
