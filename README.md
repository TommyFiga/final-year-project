# Final-Year Project
> PoC and evaluation of network restriction evasion over messaging services.


## Overview
In environments such as commercial flights, internet access is commonly sold through paid access packages. In many cases, a limited free package is also provided, allowing access only to messaging applications such as WhatsApp and Telegram, however, these applications are usually limited to text messages only.

This project explores the following question:
> _Is it possible to use authorized messaging applications as a way to bypass network limitations?_

To answer this question, this project implements a Proof of Concept (PoC) **evasion mechanism** using Telegram messages as a communication channel.

Additionally, the project implements a Machine Learning-based **detection mechanism** capable of analysing raw Wireshark traffic and classifying it as either normal traffic or evasion traffic.

> **Disclaimer:** This implementation is intended exclusively for academic purposes. There is no intention to use this type of evasion mechanism outside an academic research context.


## Objectives
### Main objectives:
- Implement the evasion mechanism
- Evaluate the viability of the evasion mechanism
- Evaluate the efficiency and security of these types of network restrictions
- Implement the detection mechanism
- Evaluate the effectiveness of the detection mechanism using multiple classifiers (e.g., kNN, SVM, Random Forest)

### Optional objectives:
- Support SOCKS5 integration for the evasion mechanism client
- Support additional methods (e.g., POST, PUT, and DELETE) for the evasion mechanism protocol